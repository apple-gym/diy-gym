...
# class ReachTargets(Addon):
#     # TODO regexp of targets?
#     def __init__(self, parent, config):
#         super(ReachTarget, self).__init__(parent, config)

#         self.source_model = parent.models[config.get('source_model')]
#         self.target_models = parent.models[config.get('target_models')]

#         self.source_frame_id = self.source_model.get_frame_id(
#             config.get('source_frame')) if 'source_frame' in config else -1
            
#         target_frames = config.get('target_frames') if 'target_frame' in config else -1 for 
#         self.target_frame_id s= self.target_model.get_frame_id(
